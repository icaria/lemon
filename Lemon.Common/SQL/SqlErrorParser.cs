...
namespace Lemon.Base
{
    ...
    public class SqlErrorParser
    {
        ...
        private void AddCommonParsers()
        {
            ...
            parsers.Add(new NegativeInventoryParser());
    ...
    internal class NegativeInventoryParser : ISqlErrorNumberParser
    {
        public SqlErrorNumber Number
        {
            get { return SqlErrorNumber.NegativeInventory; }
        }

        public ServerReturnedError ParseSqlError(SqlError error)
        {
            if (error.Number == (int)SqlErrorNumber.NegativeInventory)
            {
                ServerReturnedError e = new ServerReturnedError(ServerReturnedError.ErrorNumber.NegativeInventory);
                string regexPattern = @"Negative inventory for ItemId (?<s1>.*), ItemType (?<s2>.*), LocationId (?<s3>.*), Sublocation (?<s4>.*)";
                Regex re = new Regex(regexPattern, RegexOptions.ExplicitCapture);
                Match m = re.Match(error.Message);
                if (m.Success)
                {
                    e.Data["ItemId"] = System.Int32.Parse(m.Groups["s1"].ToString());
                    e.Data["ItemType"] = System.Int32.Parse(m.Groups["s2"].ToString());
                    e.Data["LocationId"] = System.Int32.Parse(m.Groups["s3"].ToString());
                    e.Data["Sublocation"] = m.Groups["s4"].ToString();
                }
                return e;
            }
            //Nothing found, return null
